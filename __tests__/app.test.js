...
describe("DELETE /api/articles/:article_id", () => {
  test("204: Responds with 204.", () => {
    ...
    .then(({rows}) => {
      ...
      rows.forEach((row) => {
        expect(row.article_id).not.toBe(3)
      })
    })
    .then(() => {
      return db.query(`
        SELECT users.username, CAST(COUNT(comments.comment_id)AS INT) as comment_count
        FROM users
        FULL JOIN comments
    ...
    .then(({rows}) => {
      expect(rows[1].username).toBe("icellusedkars")
      expect(rows[1].comment_count).toBe(11)
    })
  })
  test("404: Responds with page not found, when given a valid ID but no article ID is found", () => {
    return request(app)
